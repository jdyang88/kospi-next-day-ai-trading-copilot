...
앱은 `검증 AUC 0.55 이상`이고 방향 정확도가 검증기간의 단순 다수 방향 예측보다 낮지 않을 때만 매수 판정 모델을 통과시킵니다. 통과한 모델에서 상승 모델 점수 62% 이상이며 긍정적 기술 신호가 2개 이상인 종목만 `매수 추천`으로 표시합니다. 기준을 충족하는 종목이 없으면 **오늘은 매수 추천 없음**을 표시합니다. 이 판정 역시 확률적 의사결정 보조 정보이며 투자 수익을 보장하지 않습니다.
