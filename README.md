...
- 검증 기준을 통과한 경우에만 표시하는 매수 추천, 진입 기준, ATR 기반 목표가·손절가, 선정 이유
...
앱은 `검증 AUC 0.55 이상`이고 방향 정확도가 검증기간의 단순 다수 방향 예측보다 낮지 않을 때만 매수 판정 모델을 통과시킵니다. 통과한 모델에서 상승 모델 점수 62% 이상이며 긍정적 기술 신호가 2개 이상인 종목만 `매수 추천`으로 표시합니다. 기준을 충족하는 종목이 없으면 **오늘은 매수 추천 없음**을 표시합니다. 모델 결과 구조가 변경될 때는 캐시 버전을 갱신하고, 이전 캐시 객체가 남은 경우에도 매수 판정을 보수적으로 중단하도록 설계했습니다. 이 판정 역시 확률적 의사결정 보조 정보이며 투자 수익을 보장하지 않습니다.
...
새 세션의 초기 관심종목은 삼성전자, SK하이닉스, LG화학, SK이노베이션, 현대차, KODEX 200, NAVER, SK, 현대건설, 셀트리온, LS, 삼성바이오로직스, 한화에어로스페이스, 우리금융지주의 14종목입니다. 앱 사이드바의 **관심종목 설정**에서 `종목코드:종목명` 줄을 추가하거나 삭제한 뒤 적용할 수 있습니다. 호출 안정성을 위해 한 번에 최대 30종목을 분석합니다. 앱에서 편집한 목록은 현재 브라우저 세션에만 유지되므로 영구 목록 전체는 Streamlit Secrets의 `WATCHLIST`에 저장하세요.
