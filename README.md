...
- KOSPI 데모 또는 KIS 대표 10종목 실시간 스냅샷 스캔 및 Top 5 랭킹
...
- 기본 10종목과 사용자 관심종목을 합친 최대 30종목 분석
...
추천 UI는 기본적으로 데모 데이터로 안전하게 실행됩니다. Secrets가 설정된 경우 사이드바에서 **KIS 실시간 → 실시간 추천 데이터 불러오기**를 누르면 대표 10종목의 KIS 과거 일봉과 현재 OHLCV 스냅샷으로 특징·모델·Top 5를 다시 계산합니다. 최초 일봉 수집은 수 초 이상 걸릴 수 있으며 6시간 캐시되고, 현재 시세는 버튼을 누를 때 갱신됩니다. 일부 종목 조회가 실패해도 성공한 종목으로 분석을 계속합니다.

관심종목은 Streamlit Secrets의 `WATCHLIST` 또는 앱 사이드바의 **관심종목 설정**에서 `종목코드:종목명` 형식으로 추가합니다. 기본 10종목과 종목코드 기준으로 중복을 제거하며, 호출 안정성을 위해 한 번에 최대 30종목을 분석합니다. 앱에서 입력한 목록은 현재 브라우저 세션에만 유지되므로 영구 목록은 Secrets에 저장하세요.
